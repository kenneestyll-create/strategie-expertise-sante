...
"""
    ...
    results = {}
    for section_id, prompt in KIT_SECTIONS:
        user_msg = f"{context}\n\n{prompt}"
        text = None
        # PATH B — Emergent proxy stream
        try:
            text = await generate_section_llmchat(
                section_id, KIT_SYSTEM_PROMPT, user_msg, f"kit_{dossier_id}",
                ...
                try:
                    text = await asyncio.wait_for(
                        llm_call(
                            ...
                            KIT_SYSTEM_PROMPT, user_msg,
                            "anthropic", "claude-sonnet-4-5-20250929", max_tokens=2500
                        ),
                        timeout=120.0
                    )
