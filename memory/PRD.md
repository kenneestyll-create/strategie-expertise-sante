...
- **2026-05-21 — S.E.S Scene Engine V1 Sprints 1-2-3 (P1)** : refonte du moteur de rendu de la Video Factory de "texte statique sur gradient" vers **scènes motion-design** ciblées par format F1-F7. **Architecture stricte** : 100% Canvas 2D + MediaRecorder client-side, ZÉRO backend de rendu, ZÉRO WebGL/Three.js/Remotion. Source unique de vérité preview ↔ export (même classe Scene + même méthodes `update(dt, audioTime)` / `draw(ctx)`).
  ...
  - **Performance mesurée** (Playwright Chromium headless) : baseline browser idle 60.4 FPS ; F1 office_admin 59.3 FPS ; F2 stats_focus 57.8 FPS ; F3 testimony_quote 58.4 FPS ; F4 legal_balance 57.5 FPS ; tous **largement au-dessus du seuil 30 FPS cible**. Auto-degrade FPS prévue Sprint 4 reste théorique (non déclenchée sur le test).
