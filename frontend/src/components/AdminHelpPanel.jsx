...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { label: 'Personnaliser l\'email d\'invitation', text: 'Bouton "Email d\'invitation" en haut de la carte → modifiez librement l\'objet et le corps (variables {NOM}, {QUOTA}, {DATE_VALIDITE} remplacées automatiquement ; **texte** = gras ; "- " = puce). "M\'envoyer un aperçu" vous envoie le rendu exact par email avant tout envoi réel ; "Rétablir le modèle par défaut" revient au texte validé. L\'en-tête, le bouton doré et le pied de page sont ajoutés automatiquement.' },
  ...
  },
  {
    ...
    steps: [
      ...
      { label: 'V4.1 — Preview vidéo 9:16 + Voix off OpenAI TTS', text: 'NOUVEAU : chaque vidéo générée affiche désormais en tête de carte un lecteur 9:16 façon TikTok (cadre téléphone, notch, ratio strict 9:16) qui rejoue scène par scène le hook → storyboard → CTA, avec sous-titres simulés en bas. Cliquez "Lecture" pour démarrer (timeline dorée + indicateur scène + chrono). Bouton "Générer voix off" : choisissez une voix (Onyx grave/autoritaire par défaut, Sage mesurée, Alloy neutre, Nova énergique, Coral chaleureuse) puis cliquez — le backend appelle OpenAI TTS HD (modèle tts-1-hd) via Emergent LLM Key et stocke un MP3 base64 dans la vidéo. ~3-5 secondes de génération, coût ~0,003 €/vidéo. À la lecture suivante, la voix off est synchronisée à la timeline. Bouton sourdine pour couper l\'audio sans arrêter la preview. Bouton "Régénérer" pour tester une autre voix. Cette couche est 100% additive : elle ne modifie pas votre script, votre storyboard ni le CTA — c\'est une visualisation prête-à-tourner / prête-à-publier.' },
      { label: 'V4.2 — Export vidéo finale .webm (TikTok-ready, client-side)', text: 'NOUVEAU : après génération de la voix off, le bouton "📥 Exporter vidéo finale" devient actif (désactivé tant qu\'il n\'y a pas d\'audio). Cliquez : votre navigateur enregistre un canvas 720×1280 (9:16 strict) en codec VP9 via MediaRecorder, AVEC l\'audio voix-off mixé en temps réel via AudioContext, ET les sous-titres incrustés style TikTok (mode D1 : phrase complète par scène, fond noir 72% opacité, texte blanc gras 32px, highlights ✨ or #C9A84C automatiques sur les MAJUSCULES et les chiffres — ex: "CPAM", "48h", "500€"). Pendant le rendu, une barre de progression dorée s\'affiche avec le pourcentage en temps réel ("Rendering 47%"). À la fin : téléchargement automatique du fichier ses-video-{format}-{date}.webm (~3-5 Mo pour 30s). Durée max 60s (warning UI affiché si la preview dépasse, la vidéo sera tronquée). Compatible publication directe TikTok desktop, YouTube Shorts, Reels (note : Instagram Reels mobile peut nécessiter une conversion .webm→.mp4 via cloudconvert.com ou autre, 30s gratuit). 100% client-side : zéro coût serveur, zéro endpoint backend. Pré-requis navigateur : Chrome / Edge / Firefox desktop récents (Safari iOS = best-effort).' },
      { label: 'V4.3 — Marquer publié : traçabilité plateforme + URL (light tracking)', text: 'NOUVEAU : le bouton "Marquer publié" ouvre désormais un mini-dialog (et non plus une action directe). Renseignez : (1) Plateforme — Select obligatoire avec 4 options : TikTok / YouTube Shorts / Instagram Reels / Autre. (2) URL publique — Input optionnel (lien vers le post publié, ex https://tiktok.com/@ses/video/...). Cliquez "Confirmer la publication" : la vidéo passe en état publié et stocke en MongoDB : published=true, published_at (ISO), publish_platform, publish_public_url. Un badge vert "✓ Publié sur TikTok" apparaît sur la carte avec une icône de lien externe cliquable si URL renseignée (ouvre la pub dans un nouvel onglet). Le bouton devient "Re-publier" pour mettre à jour. Architecture : zéro nouveau endpoint (enrichissement du PATCH /status existant avec body optionnel rétro-compatible) — l\'ancien comportement marche encore si vous appelez l\'API sans video_idx. ZÉRO dashboard analytics : traçabilité brute uniquement, préparation des données pour V4.4 future.' },
      { label: 'V4.4 — Scene Engine V1 (motion design par format)', text: 'NOUVEAU : à partir de cette version, chaque vidéo générée reçoit un champ scene_type déterminé côté backend selon son format F1-F7 — F1/F5 → office_admin (document + tampon ✓ qui s\'appose), F2 → stats_focus (chiffre count-up + bar chart), F3 → testimony_quote (silhouette anonyme + guillemets serif Georgia), F4 → legal_balance (balance dorée qui s\'incline + article CSS sobre), F6/F7 → alert_urgency (triangle warning + flash + shake ±2-3px). Le lecteur preview ET l\'export .webm utilisent EXACTEMENT le même renderer Canvas 2D (zéro divergence). Les 9 anciennes vidéos sans scene_type basculent automatiquement sur le rendu V4.2 classique (texte + gradient) — aucune régression. Auto-degrade FPS : avant export, une probe mesure le temps de draw (10 frames) ; si > 33ms/frame en moyenne, l\'export bascule de 30 à 24 FPS pour rester stable (jamais sous 24, jamais de drop audio). Performance mesurée sur les 5 scènes : 57-60 FPS en preview, 30 FPS en export sur machine standard. Limites V1 : 1 seul scene_type par vidéo (multi-scenes en Phase 2), pas d\'asset images, pas de Three.js/WebGL.' },
      { label: 'Modèle & coûts', text: 'Claude Haiku 4.5 (Emergent LLM Key, prompt caching Anthropic). ~0,006 €/vidéo seule, ~0,011 €/vidéo+page SEO, ~0,003 €/voix off. Export .webm = 0 € (calcul navigateur). Marquer publié V4.3 = 0 € (juste une donnée MongoDB). 100 vidéos+SEO+voix/mois = ~1,40 €/mois total. Aucune API externe payante.' },
    ...
    keywords: ['video', 'vidéo', 'factory', 'tiktok', 'shorts', 'reels', 'instagram', 'youtube', 'facebook', 'hook', 'script', 'storyboard', 'srt', 'sous-titres', 'seo', 'cta', 'utm', 'haiku', 'claude', 'metrics', 'métriques', 'performance', 'poids', 'conversion', 'ctr', 'views', 'vues', 'pondération', 'exploration', 'floor', 'compliance', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'format', 'batch', 'disclaimer', 'historique', 'pack vidéo', 'production', 'editor', 'capcut', 'preview', 'voix off', 'voix-off', 'voice over', 'tts', 'openai', 'onyx', 'sage', 'alloy', 'nova', 'coral', '9:16', 'lecteur', 'player', 'v4', 'v4.1', 'v4.2', 'v4.3', 'v4.4', 'scene engine', 'scene_type', 'stats_focus', 'alert_urgency', 'legal_balance', 'office_admin', 'testimony_quote', 'motion design', 'fps', 'auto-degrade', 'canvas', 'export', 'webm', 'mediarecorder', 'vp9', 'burned-in', 'sous-titres incrustés', 'téléchargement', 'highlight', 'or', 'majuscule', 'chiffres', 'rendering', 'marquer publié', 'publication', 'plateforme', 'publish_platform', 'publish_public_url', 'tracking', 'traçabilité', 'badge']
  },
  {
    id: 'agents-org',
    tab: 'agents-org',
