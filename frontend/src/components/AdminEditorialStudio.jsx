...
const EditorView = ({ articleId, onBack, cfg }) => {
  ...
  const publish = async () => {
    if (!window.confirm("Publier cet article ? Cette action le met en ligne.")) return;
    try {
      const r = await axios.post(`${API}/admin/editorial/articles/${articleId}/publish`, {}, cfg);
      toast.success(`Publié ! URL : ${r.data.url}`);
      load();
    ...
    <div className="space-y-4" data-testid="editor-view-root">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {article.status !== 'archived' && <Button variant="outline" size="sm" onClick={archive} data-testid="editor-archive"><Archive className="w-3.5 h-3.5 mr-1.5" /> Archiver</Button>}
          <Button onClick={publish} disabled={!canPublish} className="bg-[#C9A84C] hover:bg-[#B89640] text-[#0a0a08] font-semibold" data-testid="editor-publish">
            <Send className="w-3.5 h-3.5 mr-1.5" /> Publier{unvalidated.length > 0 ? ` (${unvalidated.length} à valider)` : ''}
          </Button>
        </div>
      </div>
